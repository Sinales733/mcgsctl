...
`canvas property-readback` opens a candidate copy, selects one semantic object, enumerates every visible property-dialog tab/control, and writes `property-readback.json`, `property-dialog.tree.txt`, and `property-dialog.png`. `--probe-font` additionally opens the object's font subdialog, cancels it without saving, and records `font-dialog.tree.txt`, `font-dialog.png`, and extracted font family/style/size evidence. `property-map-probe --property-readback-dir <dir>` ingests these readbacks and prefers newer evidence for the same object ID.
