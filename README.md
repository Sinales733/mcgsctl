...
- `project.check-save`: sends save and project-check commands, then captures evidence. When it opens a project itself, it follows the workflow safety gate.
- `realtime-db.add`: adds one realtime database object through the GUI, sets base properties, saves, exports, and verifies the object name in `Data`.
- `window.button.add-momentary`: creates a standard button, configures press set-1 and release clear-0 operations, saves, exports, and verifies label/variable evidence.
...
- `script.edit`: creates a standard button, opens the script editor, writes script text, refuses secondary script-confirmation dialogs unless `--allow-create-dataobjects` is supplied, saves, exports, and verifies script tokens.
- `window.indicator.add`: creates a standard-button status indicator, configures its visibility expression, saves, exports, and verifies label/expression evidence.

## Verification Limits

`blob_strings.json` verification is evidence, not proof of full object semantics. The strongest current workflow is `device.channel.map`, which reopens the saved working copy and reads the Smart200 channel table again. Other GUI workflows still need deeper reopen/readback checks before they should be trusted for unattended production edits.
